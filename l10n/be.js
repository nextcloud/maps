OC.L10N.register(
    "maps",
    {
    "No such device" : "Няма такой прылады",
    "/Maps is not a directory" : "/Maps не з'яўляецца каталогам",
    "/Maps directory is not writeable" : "Каталог /Maps недаступны для запісу",
    "Impossible to create /Maps directory" : "Немагчыма стварыць каталог /Maps",
    "Error sharing device" : "Памылка абагульвання прылады",
    "File not found" : "Файл не знойдзены",
    "Personal" : "Асабістыя",
    "Maps" : "Карты",
    "Zoom" : "Маштаб",
    "Change color" : "Змяніць колер",
    "Export" : "Экспарт",
    "Delete" : "Выдаліць",
    "My devices" : "Мае прылады",
    "Import devices" : "Імпарт прылад",
    "Delete all" : "Выдаліць усе",
    "Cancel" : "Скасаваць",
    "Link copied" : "Спасылка скапіявана",
    "Import" : "Імпарт",
    "Category name" : "Назва катэгорыі",
    "Rename" : "Перайменаваць",
    "Copied!" : "Скапіявана!",
    "Share" : "Абагуліць",
    "My maps" : "Мае карты",
    "Clear photo cache" : "Ачысціць кэш фота",
    "Sort by name" : "Сартаваць па назве",
    "Sort by date" : "Сартаваць па даце",
    "Category" : "Катэгорыя",
    "Comment" : "Каментарый",
    "Location" : "Месцазнаходжанне",
    "Save" : "Захаваць",
    "Share this location" : "Абагуліць гэта месцазнаходжанне",
    "Shared location" : "Абагуленае месцазнаходжанне",
    "Other maps" : "Іншыя карты",
    "Close" : "Закрыць",
    "Select all" : "Выбраць усе",
    "Quit" : "Выйсці",
    "Devices" : "Прылады",
    "Favorite" : "Абранае",
    "Error while loading the file data" : "Памылка пры загрузцы даных файла",
    "Tags" : "Тэгі",
    "Duration" : "Працягласць",
    "Name" : "Назва",
    "Add" : "Дадаць",
    "Add to favorites" : "У абранае",
    "Home" : "Дадому",
    "Work" : "Праца",
    "Device" : "Прылада",
    "Date" : "Дата",
    "No name" : "Няма назвы",
    "Update" : "Абнавіць",
    "Yes" : "Так",
    "Open" : "Адкрыць",
    "Closed" : "Закрыта",
    "Search" : "Пошук",
    "File" : "Файл",
    "Start" : "Запусціць",
    "Destination" : "Прызначэнне",
    "Nextcloud additional settings" : "Дадатковыя налады Nextcloud",
    "Phone" : "Тэлефон",
    "Computer" : "Камп'ютар",
    "Unknown device type" : "Невядомы тып прылады",
    "Default" : "Прадвызначаныя",
    "Settings were successfully saved" : "Налады паспяхова захаваны",
    "Failed to save settings" : "Не ўдалося захаваць налады",
    "You are here: " : "Вы тут: ",
    "°F" : "°F",
    "Default Map" : "Прадвызначаная карта",
    "Your Default Map" : "Ваша прадвызначаная карта",
    "New Map" : "Новая карта",
    "Map deleted" : "Карта выдалена",
    "Metadata" : "Метаданыя",
    "Select folder" : "Выберыце папку",
    "Restaurant" : "Рэстаран",
    "Fast food" : "Фастфуд",
    "Bar" : "Бар",
    "Supermarket" : "Супермаркет",
    "Cafe" : "Кафэ",
    "Library" : "Бібліятэка",
    "School" : "Школа",
    "Sports centre" : "Спартыўны цэнтр",
    "Gas station" : "Заправачная станцыя",
    "Parking" : "Паркоўка",
    "Bicycle parking" : "Паркоўка для ровараў",
    "Car rental" : "Пракат аўтамабіляў",
    "ATM" : "Банкамат",
    "Pharmacy" : "Аптэка",
    "Cinema" : "Кіно",
    "Public toilets" : "Грамадскія прыбіральні",
    "Drinking water" : "Пітная вада",
    "Hospital" : "Шпіталь",
    "Doctors" : "Паліклініка",
    "Dentist" : "Стаматолаг",
    "Hotel" : "Гатэль",
    "OSRM Website" : "Вэб-сайт OSRM",
    "OSRM settings" : "Налады OSRM",
    "Maps settings" : "Налады карт"
},
"nplurals=4; plural=(n%10==1 && n%100!=11 ? 0 : n%10>=2 && n%10<=4 && (n%100<12 || n%100>14) ? 1 : n%10==0 || (n%10>=5 && n%10<=9) || (n%100>=11 && n%100<=14)? 2 : 3);");
