OC.L10N.register(
    "maps",
    {
    "Maps" : "Mapes",
    "NYI" : "NYI",
    "Click on the map to move the contact, press ESC to cancel" : "Cliqueu al mapa per moure el contacte, premeu ESC per cancel·lar",
    "Reset GEO information" : "Reiniciar informació GEO",
    "Move contact" : "Mou contacte",
    "Failed to reset contact location" : "No s'ha pogut reiniciar la ubicació del contacte",
    "Failed to load contacts" : "No s'ha pogut carregar els contactes ",
    "Failed to get contact list" : "No s'ha pogut obtenir la llista de contactes",
    "Failed to place contact" : "No s'ha pogut ubicar el contacte",
    "Import devices from gpx (Nextcloud Maps) or kml/kmz (Google Timeline) file" : "Importar dispositius des d'un fitxer gpx (Mapes de Nextcloud) o kml/kmz (Google Timeline)",
    "Failed to load device list" : "No s'ha pogut carregar la llista de dispositius",
    "Toggle history" : "Commuta l'historial",
    "Change color" : "Canvia el color",
    "Zoom to bounds" : "Zoom als límits",
    "Export" : "Exporta",
    "Delete" : "Esborra",
    "Device deleted" : "Dispositiu suprimit",
    "Failed to delete device" : "No s'ha pogut suprimir el dispositiu",
    "Failed to load device points" : "No s'ha pogut carregar els punts del dispositiu",
    "Failed to update device points" : "No s'ha pogut actualitzar els punts del dispositiu",
    "Impossible to get current location" : "No és possible obtenir la ubicació actual",
    "Failed to send current position" : "No s'ha pogut enviar la posició actual",
    "Failed to change device color" : "No s'ha pogut canviar el color del dispositiu",
    "Device" : "Dispositiu",
    "Date" : "Data",
    "Elevation" : "Elevació",
    "Accuracy" : "Precisió",
    "Battery" : "Bateria",
    "Devices exported in {path}" : "Dispositius exportats a {path}",
    "Failed to export devices" : "No s'han pogut exportar els dispositius",
    "{nb} devices imported from {path}" : "S'han importat {nb} dispositius des de {path}",
    "Failed to import devices" : "No s'han pogut importar els dispositius",
    "Personal" : "Personal",
    "Import favorites from gpx (OsmAnd, Nextcloud Maps) or kmz/kml (F-Droid Maps, Maps.me, Marble)" : "Importar favorits des d'un fitxer gpx (OSMAnd, Mapes de Nextcloud) o kmz/kml (mapes de F-Droid, Maps.me, Marble)",
    "Failed to load favorites" : "No s'han pogut carregar els favorits",
    "Add a favorite" : "Afegir un favorit",
    "Rename" : "Canvia el nom",
    "Category deleted" : "Categoria esborrada",
    "Failed to rename category" : "No s'ha pogut canviar el nom de la categoria",
    "Failed to delete category favorites" : "No s'ha pogut suprimir la categoria favorits",
    "Failed to add favorite" : "No s'ha pogut afegir el favorit",
    "Name" : "Nom",
    "No name" : "Sense nom",
    "Category" : "Categoria",
    "Comment" : "Comentari",
    "Submit" : "Enviar",
    "Favorite name" : "Nom del favorit",
    "Move" : "Moure",
    "Failed to delete favorite" : "No s'ha pogut esborrar el favorit",
    "Failed to edit favorite" : "No s'ha pogut editar el favorit",
    "Click on the map to move the favorite, press ESC to cancel" : "Cliqueu en el mapa per moure el favorit, premeu ESC per cancel·lar",
    "Favorites exported in {path}" : "S'han exportat els favorits a {path}",
    "Failed to export favorites" : "No s'han pogut exportar els favorits",
    "{nb} favorites imported from {path}" : "S'han importat {nb} des de {path}",
    "Failed to import favorites" : "No s'han pogut importar els favorits",
    "View in Maps" : "Veure a Mapes",
    "Failed to load non-geolocalized photos" : "No s'han pogut carregar les fotos no geo-localitzades",
    "Remove geo data" : "Esborrar dades GEO",
    "Click on the map to move the photo, press ESC to cancel" : "Cliqueu al mapa per moure la foto, premeu ESC per cancel·lar",
    "Failed to load photos" : "No s'han pogut carregar les fotos",
    "Choose pictures to place" : "Escolliu les imatges per ser ubicades",
    "Choose directory of pictures to place" : "Escolliu el directori d'imatges per ser ubicades",
    "{nb} photos placed" : "S'han ubicat {nb} fotos",
    "Failed to place photos" : "No s'han pogut ubicar les fotos",
    "{nb} photos reset" : "S'han reiniciat {nb} fotos",
    "Failed to reset photos coordinates" : "No s'ha pogut reiniciar les coordenades de les fotos",
    "Geo link ({geoLink}) copied to clipboard" : "S'ha copiat del porta-retalls el GEO enllaç ({geoLink})",
    "Failed to save option values" : "No s'han pogut desar els valors de les opcions",
    "Failed to restore options values" : "No s'ha pogut restaurar els valors de les opcions",
    "Place photos" : "Ubicar fotos",
    "Place photo folder" : "Ubicar una carpeta de fotos",
    "Place contact" : "Ubicar un contacte",
    "Share this location" : "Compartir aquesta localització",
    "Route from here" : "Ruta des d'aquí",
    "Add route point" : "Afegir un punt de ruta",
    "Route to here" : "Ruta fins a aquí",
    "See current location" : "Veure la ubicació actual",
    "Other layers" : "Altres capes",
    "Aerial map" : "Mapa aeri",
    "Classic map" : "Mapa clàssic",
    "north" : "nord",
    "northeast" : "nord-est",
    "east" : "est",
    "southeast" : "sud-est",
    "south" : "sud",
    "southwest" : "sud-oest",
    "west" : "oest",
    "northwest" : "nord-oest",
    "slight right" : "un poc a la dreta",
    "right" : "dreta",
    "sharp right" : "ben a la dreta",
    "slight left" : "un poc a l'esquerra",
    "left" : "esquerra",
    "sharp left" : "ben a l'esquerra",
    "Turn around" : "Gira al voltant",
    "Head {dir}" : "Dirigir-se a {dir}",
    " on {road}" : "a {road}",
    "Continue {dir}" : "Continuar {dir}",
    "Waypoint reached" : "Arribat al lloc de destí  ",
    "Take the {exitStr} exit in the roundabout" : "A la rotonda prendre la sortida {exitStr}",
    " onto {road}" : "a {road}",
    "Destination reached" : "S'ha arribat al destí",
    "At the fork, turn {modifier}" : "A la bifurcació, aneu {modifier}",
    "Merge {modifier}" : "Unir-se {modifier}",
    "Turn {modifier} on the ramp" : "Girar {modifier} a la rampa",
    "Take the ramp on the {modifier}" : "Agafeu la rampa a {modifier}",
    "Turn {modifier} at the end of the road" : "Gireu {modifier} al final de la carretera",
    "onto {road}" : "a {road}",
    "Start" : "Inici",
    "Via {viaNumber}" : "Via {viaNumber}",
    "Destination" : "Destinació",
    "m" : "m",
    "km" : "km",
    "yd" : "yd",
    "mi" : "mi",
    "h" : "h",
    "min" : "min",
    "s" : "s",
    "Routing error:" : "Error d'encaminament:",
    "Route exported in {path}" : "S'ha exportat la ruta a {path}",
    "Failed to export current route" : "No s'ha pogut exportar la ruta actual",
    "No search result" : "No s'han trobat resultats",
    "No {POItypeName} found" : "No s'ha trobat {POItypeName}",
    "My location" : "La meva ubicació",
    "Restaurant" : "Restaurant",
    "Fast food" : "Fast-food",
    "Bar" : "Bar",
    "Supermarket" : "Supermercat",
    "Cafe" : "Cafè",
    "Library" : "Biblioteca",
    "School" : "Escola",
    "Sports centre" : "Centre esportiu",
    "Gas station" : "Benzinera",
    "Parking" : "Estacionament",
    "Bicycle parking" : "Estacionament de bicis",
    "Car rental" : "Lloguer de vehicles",
    "ATM" : "Caixer",
    "Pharmacy" : "Farmàcia",
    "Cinema" : "Sala de cinema",
    "Public toilets" : "Banys públics",
    "Drinking water" : "Aigua potable",
    "Hospital" : "Hospital",
    "Doctors" : "Doctors",
    "Dentist" : "Dentista",
    "Hotel" : "Hotel",
    "Add to favorites" : "Afegir a favorits",
    "Open" : "Obert",
    "closes in {nb} minutes" : "tanca en {nb} minuts",
    "until {date}" : "fins {date}",
    "Closed" : "Tancat",
    "opens at {date}" : "obre el {date}",
    "Load gpx file" : "Carregar el fitxer gpx",
    "Load gpx files from directory" : "Carregar els fitxers gpx des del directori",
    "Close elevation chart" : "Tancar el gràfic de desnivell",
    "Failed to remove track" : "No s'ha pogut suprimir el trajecte",
    "Failed to add track directory" : "No s'ha pogut afegir el directori de trajectes",
    "Failed to add tracks" : "No s'han pogut afegir els trajectes",
    "Change track color" : "Canviar el color del trajecte",
    "Show track elevation" : "Mostrar el desnivell del trajecte",
    "Remove" : "Suprimir",
    "Failed to load tracks" : "No s'han pogut carregar els trajectes",
    "Failed to load track content" : "No s'ha pogut carregar el contingut del trajecte",
    "Failed to parse track {fname}" : "No s'ha pogut analitzar el trajecte {fname}",
    "File" : "Fitxer",
    "Link" : "Enllaç",
    "Latitude" : "Latitud",
    "Longitude" : "Longitud",
    "Description" : "Descripció",
    "Symbol name" : "Nom del símbol",
    "download" : "baixar",
    "metadata link" : "enllaç de metadades",
    "tracks/routes name list" : "llista de noms de trajectes/rutes",
    "no name" : "sense nom",
    "Distance" : "Distància",
    "Duration" : "Durada",
    "Moving time" : "Temps en moviment",
    "Pause time" : "Temps de pausa",
    "no date" : "sense data",
    "Begin" : "Començar",
    "End" : "Final",
    "Cumulative elevation gain" : "Desnivell positiu acumulat",
    "Cumulative elevation loss" : "Desnivell negatiu acumulat",
    "Minimum elevation" : "Altitud mínima",
    "Maximum elevation" : "Altitud màxima",
    "Maximum speed" : "Velocitat màxima",
    "Average speed" : "Velocitat mitjana",
    "Moving average speed" : "Mitjana mòbil de velocitat",
    "Moving average pace" : "Mitjana mòbil de ritme",
    "Show elevation" : "Mostrar altitud",
    "Failed to change track color" : "No s'ha pogut canviar el color del trajecte",
    "Simple maps app for Nextcloud" : "Una aplicació senzilla de Mapes per Nextcloud",
    "Simple maps app to display a leaflet map." : "Una aplicació senzilla per veure mapes.",
    "Search…" : "Cercar...",
    "Favorites" : "Favorits",
    "Import from gpx/kml/kmz" : "Importar des de gpx/kml/kmz",
    "Show all" : "Veure tot",
    "Hide all" : "Amagar tot",
    "Routing" : "Encaminament",
    "Export current route to gpx" : "Exportar aquesta ruta a gpx",
    "Photos" : "Fotos",
    "Nothing yet" : "Encara res",
    "without geo tag" : "sense etiquetes GEO",
    "Save all visibile" : "Deseu tot el que és visible",
    "Contacts" : "Contactes",
    "Tracks" : "Trajectes",
    "Load a track file" : "Carregar un fitxer de trajecte",
    "Load a directory" : "Carregar un directori",
    "Remove all" : "Suprimir tot",
    "Devices" : "Dispositius",
    "Refresh positions" : "Refrescar les posicions",
    "Export all" : "Exportar-ho tot",
    "Import devices" : "Importar dispositius",
    "Delete all" : "Esborrar tot",
    "Settings" : "Paràmetres",
    "Track my position" : "Rastreja la meva posició"
},
"nplurals=2; plural=(n != 1);");
