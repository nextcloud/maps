...
export const Axis = ({
	...
	return g => {
		...
		const axisScale = d3['axis' + position.replace(/\b\w/g, l => l.toUpperCase())]()
			.scale(scale)
...
export const LegendSmall = ({
	...
	return g => {
		...
		const symbol = g.selectAll('.legend-switcher-symbol').data([
...
export const Ruler = ({
	...
	return g => {
		...
		const rect = g.selectAll('.horizontal-drag-rect').data([{ w: width }])
		const line = g.selectAll('.horizontal-drag-line').data([{ w: width }])
		const label = g.selectAll('.horizontal-drag-label').data([{ w: width - 8 }])
		const symbol = g.selectAll('.horizontal-drag-symbol')
			.data([{
...
export const Domain = ({
	...
	const domain = data && data.length ? d3.extent(data, d => d[attr]) : [0, 1]
...
export const Chart = ({
	...
	// Add the brushing
	const brush = d3.brushX().on('start.cursor end.cursor brush.cursor', () => panes.brush.select('.overlay').attr('cursor', null))
