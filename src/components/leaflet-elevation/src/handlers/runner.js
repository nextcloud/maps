...
export async function Runner() {
	...
	runnerOptions = L.extend(
		...
		typeof runnerOptions === 'object' ? runnerOptions : {},
	)
	...
	this.addRunner = function(runner) {
		const x = this._x(runner.dist)
		const y = this._y(this._findItemForX(x).z)
	...
	this.setPositionFromLatLng = function(latlng) {
		this._onMouseMoveLayer({ latlng }) // Update map and chart "markers" from latlng
	}
	...
	this.animate = function(layer, speed = 1500) {
		...
		this._runner = L.motion.polyline(
			...
			L.extend({}, { auto: true, speed }, runnerOptions.motion),
