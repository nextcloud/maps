export function Slope() {
	...
	return {
		...
		pointToAttr: (_, i) => { // slope in % = ( dy / dx ) * 100;
			const dx = (this._data[i].dist - this._data[i > 0 ? i - 1 : i].dist) * 1000
			const dy = this._data[i][this.options.yAttr] - this._data[i > 0 ? i - 1 : i][this.options.yAttr]
			return dx !== 0 ? (dy / dx) * 100 : NaN
		},
		onPointAdded: (_, i) => {
			const dz = this._data[i][this.options.yAttr] - this._data[i > 0 ? i - 1 : i][this.options.yAttr]
