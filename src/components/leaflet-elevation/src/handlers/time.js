	...
	return {
		...
		pointToAttr(point, i) {
			...
			if (!point.meta || !point.meta.time) {
				...
				if (i > 0) {
					const dx = (this._data[i].dist - this._data[i - 1].dist)
					const t0 = this._data[i - 1].time.getTime()
			...
			const time = (point.meta.time.getTime() - point.meta.time.getTimezoneOffset() * 60 * 1000 !== 0) ? point.meta.time : 0
			// Update duration
